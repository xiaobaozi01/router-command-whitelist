...
命令编辑器提供两个 AI 能力：在不改变命令文字的前提下标注固定关键字、可替换参数和废弃内容；根据命令表达式生成 Java 正则以及正反测试数据。管理员还可以在待审批详情中调用 AI 审批助手，查看变更摘要、风险等级、风险点、核对清单和审批建议。首尾匹配边界始终由用户和系统控制，AI 不会修改；AI 审批结论也只供参考，不会自动通过或驳回申请。AI 结果会先预览，用户确认后才写入表单；后端还会检查命令原文和自动运行生成的正反例。
